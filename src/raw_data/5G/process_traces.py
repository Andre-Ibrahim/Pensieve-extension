...
output_file = 'time_dl_bandwidth.csv'

for i, dir_path in enumerate(current_directory.iterdir()):
    if dir_path.is_dir():
        for j, file_path in enumerate(dir_path.iterdir()):
            if file_path.is_file() and file_path.name != "Amazon_Prime.DS_Store":
                
                print(dir_path.name + file_path.name)
                ...
                time_dl_df['DL_bitrate'] = time_dl_df['DL_bitrate'].astype('float64')
                time_dl_df.loc[:, 'DL_bitrate'] = time_dl_df['DL_bitrate'] / 1000
